...
export default function LearnersTable() {
  ...
  // Add new learner from modal
  const handleLearnerAdded = (newLearner: Learner) => {
    setLearners((prev) => [...prev, newLearner]);
  };

  const renderCell = useCallback((learner: Learner, columnKey: string) => {
    switch (columnKey) {
      case "name":
        return (
          <User
  ...
  return (
    <>
      {/* Add Learner Button + Modal */}
      <LearnersModal onLearnerAdded={handleLearnerAdded} />

      {/* Table */}
      <Table aria-label="Learners table with actions">
      ...
      {selectedLearner && (
        <MarksModal
          ...
          onCloseModal={() => setSelectedLearner(null)}
        />
      )}
    </>
  );
